...
// Configurações para melhorar a qualidade e desempenho
const DEFAULT_FRAME_RATE = 10; // Aumentado para melhor fluidez
const DEFAULT_QUALITY = 0.7; // Aumentado para melhor qualidade (0-1)
...
class SocketStreamPeer extends EventEmitter {
  constructor(options = {}) {
    ...
    this._context = this._canvas.getContext("2d", { alpha: false }); // Desativar alpha para melhor desempenho
    this._videoElement = document.createElement("video");
    this._frameInterval = null;
    this._frameRate = options.frameRate || DEFAULT_FRAME_RATE;
    this._quality = options.quality || DEFAULT_QUALITY;
  ...
  _startSendingFrames() {
    ...
    this._videoElement.addEventListener("loadedmetadata", () => {
      ...
      console.log(
        `Configurado canvas com dimensões ${this._canvas.width}x${this._canvas.height}`
      );
  ...
  _captureAndSendFrame() {
    ...
    // Controle de taxa de frames - limitar envio se estiver muito rápido
    const now = Date.now();
    const elapsed = now - this._lastFrameTime;
    const minInterval = 1000 / this._frameRate;

    if (elapsed < minInterval) {
      return; // Pular este frame para manter a taxa desejada
    }
    ...
    try {
      ...
      // Converter o canvas para uma imagem JPEG com a qualidade configurada
      const frameData = this._canvas.toDataURL("image/jpeg", this._quality);
  ...
  _handleVideoFrame(frameData) {
    ...
    if (!this._videoTrack) {
      ...
      img.onload = () => {
        ...
        // Criar um stream a partir do canvas com a taxa de frames configurada
        const stream = this._canvas.captureStream(this._frameRate);
        this._videoTrack = stream.getVideoTracks()[0];

        // Configurar o nome da faixa de vídeo para melhor identificação
        if (this._videoTrack && this._videoTrack.label) {
          this._videoTrack.label = "Socket.IO Stream";
        }

        console.log("Stream criado a partir do canvas com sucesso");
  ...
  signal(data) {
    ...
    console.log(
      "Método signal chamado, mas não é necessário para SocketStreamPeer"
    );
  }
